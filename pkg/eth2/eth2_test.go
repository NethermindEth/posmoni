...
func setup(data [][]net.ValidatorBalance) (*eth2Monitor, error) {
	...
	return NewEth2Monitor(&db.SQLiteRepository{DB: ormdb}, newTestBeaconClient(data), net.SubscribeOpts{}), nil
}
...
func TestGetValidatorBalance(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			monitor, err := setup(tc.requestData)
			if err != nil {
